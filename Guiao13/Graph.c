...
Graph* GraphCopy(const Graph* g) {
  Graph* g2;
  if(g->isComplete){
    g2 = GraphCreateComplete(g->numVertices,g->isDigraph);
  }else{
    g2 = GraphCreate(g->numVertices,g->isDigraph,g->isWeighted);
  }
  return g2;
}

Graph* GraphFromFile(FILE f) {
  unsigned int digrafo = getw(&f);
  unsigned int peso = getw(&f);
  unsigned int numVertices = getw(&f);
  unsigned int numEdge= getw(&f);
  Graph* a = GraphCreate(numVertices,digrafo,peso);
  for(int i = 0; i < numEdge; i++){
    unsigned int v = getw(&f);
    unsigned int w = getw(&f);
    if(peso){
      double tmp;
      fscanf(&f,"%lf",&tmp);
      int weight =(int) tmp*100;
      GraphAddWeightedEdge(a,v,w,weight);
    }else{
      GraphAddEdge(a, v, w);
    }
  }
